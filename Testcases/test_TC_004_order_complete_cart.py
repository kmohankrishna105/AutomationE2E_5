...
def test_login():
    ...
    url=Configreader.readElement("login", "sit_url")
    driver.get(url)
    ...
    driver.execute_script('scrollBy(0,200)')
    driver.find_element(By.XPATH, "(//span[@class='price'])[3]").click()
    driver.find_element(By.XPATH, "(//div[@class ='primary']/button[@ type='submit'])[2]").click()

    err_msg = "//*[*='This is a required field.']"

    try:
        if driver.find_element(By.XPATH, err_msg):
            print("Cannot procceed further")
    except:
        print("No Errors -> Will be able to proceed further")
    print("Same")
    driver.execute_script('scrollBy(0,300)')
    bill_address = "//div[@class='billing-address-details']"
    inp = input("Please check")
    address = driver.find_element(By.XPATH, bill_address).text
    print(address)

    ord_total = "//td[@data-th='Order Total']//span[@class='price']"
    ord_total_value = driver.find_element(By.XPATH, ord_total).text
    print(ord_total_value)

    place_order_button = "// button[@title='Place Order']/span"
    driver.find_element(By.XPATH, place_order_button).click()
    time.sleep(5)
    ord_num = "//div[@class='checkout-success']//a/strong"
    order_num = driver.find_element(By.XPATH, ord_num).text
    print(order_num)
